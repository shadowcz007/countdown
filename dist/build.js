...
var topics = ['未来城市', '视频', '工具', '人工智能辅助设计', '智能设计', '色彩', '字体', '游戏', '因果关系', '归因', '数据科学', '前端智能', '虚拟偶像', '艺术', '体育', '舞蹈', '摄影', '法律', '美食', '社交', '生物黑客', '教育', '职业规划', 'AR', 'VR', '谷歌', '设计', '宁静技术', '三维重建', '智能产品'];
var topicsDom = document.getElementById("topics");
Array.from(topics, function (t) {
    var c = createTag(t);
...
function getDataAndStart(tag) {
    var items;
    return regeneratorRuntime.async(function getDataAndStart$(_context) {
        while (1) {
            switch (_context.prev = _context.next) {
                case 0:
                    _context.next = 2;
                    return regeneratorRuntime.awrap(getData(tag));
...
//随机重排
var oldItems = Array.from(document.querySelectorAll('.item'), function (t) {
    return t;
});
var newItems = [];
var len = oldItems.length;
for (var i = 0; i < len; i++) {
    var index = Math.floor(Math.random() * oldItems.length);
    newItems.push(oldItems[index]);
    oldItems.splice(index, 1);
};
document.body.querySelector('main').innerHTML = "";
Array.from(newItems, function (t) {
    return document.body.querySelector('main').appendChild(t);
});
...
function start(items) {
    ...
    tags_list = tags_list.sort(function (b, a) {
        return a.value - b.value;
    });
    tags_list = Array.from(tags_list, function (t) {
        return t.key;
    }).slice(0, 10);

    var tagsDom = document.querySelector("#tags");
    tagsDom.innerHTML = "";

    Array.from(tags_list, function (t) {
        t = createTag(t, function (e) {
            console.log(e.target.innerText);
            var text = e.target.innerText;
            Array.from(document.querySelectorAll(".content"), function (c) {
    ...
    */
    ...
    var cardsDom = document.querySelector("#cards");
    cardsDom.innerHTML = "";
...
function createTags(tags) {
    return "<div class=\"tags\">" + Array.from(tags, function (t) {
        return createTag(t).outerHTML;
    }).join("") + "</div>";
}
...
function createURLs(urls) {
    return "<div class=\"urls\">" + Array.from(urls, function (u) {
            ...
            switch (_context2.prev = _context2.next) {
                case 0:
                    return _context2.abrupt("return", new Promise(function (resolve, reject) {
                        fetch('data/' + t + '.json').then(function (r) {
                            return r.json();
                        }).then(function (r) {
                            //console.log(r);
                            resolve(r);
